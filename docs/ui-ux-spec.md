...
折叠/展开规则:
- 所有状态默认折叠（仅显示头部），用户点击头部展开查看详情
- 展开时箭头 ▾ 指向下，折叠时 ▶ 指向右
- AWAITING_APPROVAL：始终展开，不可折叠（头部 cursor=default，箭头隐藏）
- EXECUTING：不可折叠（用户需要看到进度条和终止按钮）
- 结果文本区域 max-height=240px，超出滚动显示
```
...
// PlanCard 不提供独立的暂停/继续/删除计划按钮。暂停跟随全局发送/停止按钮（用户点发送按钮后变为停止按钮 ⏹，点击停止即暂停）。
// PlanCard 仅展示步骤进度 + PENDING 步骤行末 ✕ 单步删除入口。
// LLM 通过 listSteps/deleteStep/reorderSteps 工具自主管理任务列表。
```
...
### 2.11 多 Agent 调度卡片（MultiAgentBlock）
...
折叠态（仅显示头部 + 子 Agent 状态行）:
┌──────────────────────────────────────────┐
│ ▶ 🤖 多 Agent 调度中           2/3 运行中│
│ 🔵 A: 重构 UserService    ✅  完成       │
│ 🔵 B: 更新 UserController  🔄  执行      │
│ ⏸ C: 运行测试               ⏸  排队      │
└──────────────────────────────────────────┘
...
- 排队的子 Agent 不可展开（无内容）
- 嵌套 ToolCallCard 遵循与父 ToolCallCard 相同的折叠规则
- 文件锁持有者标注在底部状态行

颜色:
- 卡片背景: bg=#F0F7FF (亮) / #0F1D2F (暗), border=#BFDBFE
- 完成: #22C55E, 执行中: #3B82F6, 排队: #6B7280
```
...
PlanCard 交互规则:
  ...
  默认自动连续执行:
    ...
    └─ 用户点全局停止按钮(⏹) → 完成当前 Step 后停止
  
  PlanCard 仅负责展示:
    └─ 步骤列表及状态（含 PENDING 步骤行末 ✕ 单步删除）
