...
├──────────┼─────────────────────────────────┤
│ Disabled │ fg=#D1D5DB, cursor=default      │
│          │ (Welcome 页面时，除 Settings 外  │
│          │  全部禁用)                       │
├──────────┼─────────────────────────────────┤
│ Badge    │ 右上角角标                       │
│          │ bg=#EF4444, fg=#FFFFFF, 8px 圆   │
│          │ 数字 > 99 → "99+"               │
│          │ 仅 Sessions 页面有               │
└──────────┴─────────────────────────────────┘
```
...
├───────────────┼──────────┼──────────────────┤
│ AWAITING_     │ 🔒 黄色   │ 等待用户审批     │
│ APPROVAL      │ icon=Warning│ [批准] [拒绝]  │
│               │ bg=#FFFBEB│ 无超时，等待用户操作       │
├───────────────┼──────────┼──────────────────┤
│ EXECUTING     │ 🔄 旋转   │ 执行中           │
│               │ icon=Step_4│ 不可折叠        │
│               │ 进度条    │ [⏹ 终止]         │
...
Plan 执行中流转:
  暂停 → 用户可:
    ├─ 切换到 Sessions 页面查看其他会话
    ├─ 切换到 Token Usage 页面查看消耗
    ├─ 在同一 Session 内发送聊天消息 (不恢复计划)
    ├─ 输入 "继续" → resumeNextStep()
    ├─ 输入 "重试 Step N" → retryStep(N)
    ├─ 输入 "跳过 Step N" → skipStep(N)
    └─ 输入 "取消计划" → abortPlan()
```
...
| 面板宽度            | 布局调整                                                    |
|-----------------|---------------------------------------------------------|
| < 300px         | 气泡 maxWidth=90%。User 气泡和 Agent 气泡都占满。ToolCallCard 参数行换行 |
| 300-500px       | 默认行为。气泡 maxWidth=70%(用户)/85%(Agent)                     |
| > 500px         | 气泡 maxWidth=60%(用户)/75%(Agent)。代码块可并排显示（如果有多个短代码块）      |
| ToolWindow 浮动模式 | 同 < 300px 行为                                            |
...
| 焦点指示器 | Focus 时输入框边框加粗到 2px+变色。按钮 focus 时虚线外框（默认 L&F）                                |
