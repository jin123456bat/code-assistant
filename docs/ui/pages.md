...
**TabBar 显隐规则：** 无 API Key 时，TabBar 仅显示 Welcome（🏠），其他 Tab 通过 Swing `setVisible(false)`
隐藏。保存
...
**会话标题生成：** 详见 [Session 管理](../agent/session.md#会话标题生成)。
...
- 搜索：session title `String.contains()` 文本过滤 + 前端 debounce 300ms，无结果时显示空状态并隐藏底部操作栏
...
- **子任务聚合**：父 session 的 token 统计通过 `SessionManager.aggregateChildTokens()` 递归聚合所有子
  session 的
  token（`parentTotalTokens` 字段）
...
- 状态指示灯：🟢 RUNNING / 🟡 INITIALIZING / 🔴 CRASHED / 🔴 INIT_ERROR / 🟡 DISCONNECTED / ⚪ STOPPED / ⚠️
  Schema 校验失败
- Schema 校验失败的工具在卡片上显示 ⚠️ 标记 + 失败原因
...
- 工具交叉验证：声明但未注册的工具 → ⚠️ 工具缺失；command 与其他 Skill 冲突 → ⚠️ command 冲突
- `hasMissingTools=true` 或有冲突的 Skill 不可调用
