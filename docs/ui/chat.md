...
**流式渲染策略：** 首 token 即时渲染（让用户立刻看到响应），后续 token 写入 buffer 并重置 30ms
静默计时器。token 连续到达时 timer 被不断推迟，直到 LLM 停顿 ≥30ms 后才将积压的一批 token 一次性 flush
到 UI。未闭合 Markdown
...
- 静默合并 flush（token 停顿 ≥30ms 后批量渲染）→ 增量更新最后一段
- 未闭合代码块 → 缓存等待
...
│   → 流式 Markdown → BlockSequence（首 token 即时，后续静默合并 ≥30ms flush）
│
└── BlockSequence:
    ├── 内部结构：List<Block>
