...
- 页面顶部错误横幅（banner）：持续可见，认证错误显示 Settings 入口。详见 [横幅](#十toast--横幅)。
- 消息流错误气泡：保留错误文本，并提供 `[重试]`（存在可重试 turn 时）和 `[复制]` 操作。
...
> 8 个状态、折叠/展开规则、Diff
> 可视化等详细规范见 [工具系统 §十](../agent/tools.md#十toolcallcard-ui-状态)。
