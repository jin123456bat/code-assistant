...
**流式渲染策略：** 每个 token 到达后直接通过 `invokeLater` 追加到 JTextPane，由 Swing
RepaintManager 自行合并重绘。未闭合 Markdown 块（如未配对的 ```）缓存等待，闭合后再渲染。
`MessageAccumulator`
自动累积完整 Message 用于持久化。
...
- 每个 token 到达后 invokeLater 追加到 JTextPane，Swing RepaintManager 合并重绘
- 未闭合代码块 → 缓存等待
...
│   → 流式 Markdown → BlockSequence（每个 token 通过 invokeLater 追加，Swing RepaintManager 合并重绘）
│
└── BlockSequence:
    ├── 内部结构：List<Block>
