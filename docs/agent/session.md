...
- **跨进程锁**：`FileChannel.tryLock()` 获取 OS 级排他锁（跨 JVM 有效），获取失败等 100ms 重试最多 3
  次，应对多 IDE 窗口场景
- **Session Index 同样保护**：`index.json` 同样走 `.tmp → ATOMIC_MOVE` + `FileLock`
  ，多窗口并发写入时后写入者覆盖（最后写入胜出），不会出现文件损坏
...
30 天按日折线图，Custom JComponent `paintComponent` 手绘。TokenUsagePage 只显示 token 使用量，不显示具体价格：

- X 轴 = 日期（1 日~30 日）
- Y 轴 = 当日 totalTokens（input+output），单位 K（千）/ M（百万）
- 仅显示趋势线，省略坐标轴标签
- 鼠标 hover 显示具体数值
...
│  按会话：                                       │
│  ┌──────────────────────────────────┬───────────┐│
│  │ 📝 重构 UserService        ⏸    │ 45.0K     ││ ← 点击跳转到该会话
│  │ 💬 Kotlin 协程讨论              │  3.0K     ││
│  └──────────────────────────────────┴───────────┘│
└──────────────────────────────────────────────────┘
```

Token 使用量使用 K（千）、M（百万）为单位，不显示具体价格。
