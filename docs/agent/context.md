...
| Skill 正文（`/command` 触发时） | **从磁盘重新注入**：compact 后通过 `builder.addSystemMessage()` 以 system message 角色重新注入已调用过的 Skill 正文，位于压缩摘要和近期消息之间。确保 LLM 在 compact 后仍能访问 Skill 能力，但不膨胀 System Prompt |
