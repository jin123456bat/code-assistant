...
| Agent           | Agent           | AgentLoop 复用        | 启动子代理异步处理子任务，完成后回调通知父 Agent              |
| WebSearch       | WebSearch       | HTTP API            | 搜索网页，返回标题+URL 列表                         |
| WebFetch        | WebFetch        | HTTP API            | 抓取 URL 内容并按提示提取信息                        |
| AskUserQuestion | AskUserQuestion | IDE Dialog          | 向用户发起多选/单选问题以澄清需求                        |
| Symbol          | —               | PSI + StubIndex     | 代码导航（8 种操作）：跳转定义/实现、查引用、类型提示、文件/全局符号、调用链 |
