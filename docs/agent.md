...
| [agent/tools.md](agent/tools.md)             | 工具系统：14 个工具（9 内置 + 5 计划管理）+ 执行分发 + 审批 + Shell 安全 + 前置校验         |
| [agent/plan.md](agent/plan.md)               | Plan Mode：自动连续执行 + LLM 自主管理 + PlanCard/PlanExecutor + 边界处理      |
| [agent/multi-agent.md](agent/multi-agent.md) | 多 Agent 协作：Task 工具 + MultiAgentManager + 并发控制 + 文件写锁            |
...
│  Read/Write/Edit/Bash/Glob/Grep/ReadLints/Task/Skill    │
...
| Task      | Task      | AgentLoop 复用        | 启动子代理处理子任务                             |
...
状态：IDLE | PROCESSING | AWAITING_APPROVAL | EXECUTING | CANCELLED | ERROR | PAUSED
...
| Agent 最大轮次   | 20（0=不限）          | 达到上限后自动终止。每轮 = 一次用户消息触发的 API 调用（续写不增加 turn） |
| 多 Agent 并发上限 | 3                 | 父 + 子 Agent 总计并发数                           |

## 设计决策记录

以下是 Agent 模式设计过程中的关键决策（来源：DESIGN.md 十一）：
...
| `agent/`   | `ToolModels.kt`           | 14 个 Tool 数据类 + @JsonClassDescription 注解         |
