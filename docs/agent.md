...
| [agent/tools.md](agent/tools.md)                                     | 工具系统：8 个内置工具 + 执行分发 + 审批 + Shell 安全 + 前置校验                                   |
| [agent/markdown-rendering.md](agent/markdown-rendering.md)           | Markdown 渲染：手写解析器 + 5 种 Block 类型 + Swing 组件映射                                |
| [agent/plan.md](agent/plan.md)                                       | Plan Mode：自动连续执行 + LLM 自主管理 + PlanCard/PlanExecutor + 边界处理                   |
| [agent/multi-agent.md](agent/multi-agent.md)                         | 多 Agent 协作：Agent 工具 + MultiAgentManager + 并发控制 + 子 Agent 审批策略 + 工具白名单 + 文件写锁 |
...
| [agent/mcp.md](agent/mcp.md)                                         | MCP 支持：JSON-RPC via stdio/HTTP + Server 生命周期 + 崩溃恢复                          |
...
| [specs/multi-project-isolation.md](specs/multi-project-isolation.md) | 多项目隔离：同项目多窗口冲突警告                                                             |
| [specs/modification-stamp.md](specs/modification-stamp.md)           | 文件写锁：乐观锁 stamp 检测 + 冲突处理                                                     |

## 跨模块规范

| 文档                                                           | 说明                                               |
|--------------------------------------------------------------|--------------------------------------------------|
| [specs/module-interaction.md](specs/module-interaction.md)   | **模块交互总图**：依赖全景图 + 关键接口契约 + 核心调用时序               |
...
| [specs/plugin-lifecycle.md](specs/plugin-lifecycle.md)       | 插件启动与初始化流程                                       |
| [specs/api-error-handling.md](specs/api-error-handling.md)   | API 错误码处理策略：Agent / Completion / Git Message     |
| [specs/bash-security.md](specs/bash-security.md)             | Bash 安全模型：审批策略 + 执行环境 + 超时限制                     |
| [specs/mcp-permissions.md](specs/mcp-permissions.md)         | MCP 权限模型：Server 生命周期 + 权限边界 + 崩溃恢复               |
| [specs/auto-compact.md](specs/auto-compact.md)               | Auto-Compact 上下文压缩：System Prompt + 摘要 Prompt     |
| [specs/tool-json-schema.md](specs/tool-json-schema.md)       | 工具 JSON Schema 完整定义（13 内置 + 5 计划管理）              |
| [specs/testing.md](specs/testing.md)                         | 测试策略：三层测试 + 覆盖目标 + CI 集成                         |
| [specs/i18n.md](specs/i18n.md)                               | 国际化策略：中英双语，ResourceBundle + Locale 回退            |
...
│  CardLayout 容器切换页面                                 │
├──────────────────────────────────────────────────────┤
│  Chat UI: JLabel (Markdown 段落/标题/列表) + JTextArea (代码块) │
...
│  MultiAgentManager                                     │
├──────────────────────────────────────────────────────┤
│  Tool Layer: ToolRegistry（12 个内置工具 + 5 个计划管理工具）    │
│  Read/Write/Edit/Bash/Glob/Grep/readLints/Agent/Skill/        │
...
共 13 个内置工具 + 5 个计划管理工具。以 [
`specs/tool-return-formats.md`](specs/tool-return-formats.md) 为权威参考（定义 LLM
看到的返回值格式），完整参数、上限、执行线程、审批规则、JSON Schema 详见 [
...
- LLM 通过 `createPlan` 工具主动创建计划
- 超长任务通过 createPlan 预防 + 轮次预警 + Auto-Compact 兜底管理，复杂度自判
- MCP `resources/list` 和 `prompts/list` 不支持
- 多 Agent 嵌套上限 1 层（子不可再 spawn 孙）
- Sessions 搜索仅支持 title 过滤
...
| Markdown 渲染   | 手写字符串解析器，5 种 Block 类型                       | 纯字符串操作无 EDT 约束，详见 [markdown-rendering.md](agent/markdown-rendering.md) |
...
| 会话持久化         | Gson → .tmp → ATOMIC_MOVE + FileLock        | 防写入中断数据丢失 + 多实例竞态                                                      |
...
| `agent/`   | `ToolModels.kt`           | 18 个 Tool 数据类（13 内置工具 + 5 计划管理工具）+ @JsonClassDescription 注解 |
...
| `ui/`      | `RoundedBorder.kt`        | 圆角边框绘制工具                                                    |
| `ui/`      | `SimpleDiff.kt`           | LCS diff 算法                                                 |
...
- Markdown 解析：手写字符串解析器（5 种 Block 类型），详见 [
