...
| [agent/multi-agent.md](agent/multi-agent.md) | 多 Agent 协作：Task 工具 + MultiAgentManager + 并发控制 + 子代理审批策略 + 工具白名单 + 文件写锁 |
...
| [agent/review.md](agent/review.md)           | 🔍 文档审查报告：设计缺陷 + 代码不一致 + 修复方案（2026-06-28）                              |
...
| 工具           | 说明                                      |
|--------------|-----------------------------------------|
| createPlan   | 创建执行计划，最多 20 项。计划创建后自动开始执行，LLM 可用其他工具管理 |
