...
截断（> 10 条）:
找到 {totalCount} 条搜索结果，已返回 10 条（第 {offset+1}-{offset+10} 条）:
{results}
... (共 {totalCount} 条，已返回 10 条。用 offset={nextOffset} 翻页获取更多)
...
上限：≤ 10 条/页，超出时用 `offset` 参数翻页。不支持缓存。
...
限制：HTTP 自动升级为 HTTPS。跨主机重定向返回给 LLM 而非自动跟随。不支持缓存。
